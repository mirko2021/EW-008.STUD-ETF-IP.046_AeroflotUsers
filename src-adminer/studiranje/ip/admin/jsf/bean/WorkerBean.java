...
import programiranje.ip.admin.model.Worker;
...
/**
 * Администраторско зрно за баратање запосленим радницима. 
 * Администратор може избрисати, измјенити или додати запосленог. 
 * @author VM
 * @version 1.0
 */
@ManagedBean(name = "workerAdminBean", eager = true)
@SessionScoped
public class WorkerBean implements Serializable{
	...
	public List<Worker> list(){
		try {
			...
			for(var data : AppLogicCenter.appLogicCenter.getEngine().getDataSource().list(page)) {
				...
				if(roleDAO.getWorkerRole(data.getCredentials().getUser().getUsername())==null) continue; 
				if(!roleDAO.getWorkerRole(data.getCredentials().getUser().getUsername()).getValue().contentEquals("true"))
					continue; 
				Worker w = new Worker();
	...
	public void revokeRole() {
	    try {
			...
			messageBean.setMessage("Повлачење привилегије запосленог за "+userName+" је успјешно.");
		}catch(Exception ex) {
			MessageBean messageBean = DesignBeansGenerator.messageBean(session); 
			messageBean.setType(MessageType.ERROR);
			messageBean.setException(ex);
			messageBean.setMessage("Повлачење привилегије запосленог за "+userName+" није успјешно.");
		}
	}
	...
	public void updateData() {
		try {
			...
			messageBean.setType(MessageType.SUCCESS);
			messageBean.setException(null);
			messageBean.setMessage("Измјена основних података за запосленог "+userName+" је успјешна.");
		}catch(Exception ex) {
			MessageBean messageBean = DesignBeansGenerator.messageBean(session); 
			messageBean.setType(MessageType.ERROR);
			messageBean.setException(ex);
			messageBean.setMessage("Измјена основних података за запосленог "+userName+" није успјешна.");
		}
	}
	...
	public void updateUsername() {
		try {
			...
			UserCredentialsController ucc = new UserCredentialsController();
			Worker neo = getWorker();
			
			if(neo==null) throw new RuntimeException("New worker data not found");
			if(!neo.getUserName().contentEquals(w.getUserName())) {
				if(AppLogicCenter.appLogicCenter.getEngine().getDataSource().get(neo.getUserName())!=null)
					throw new RuntimeException("New worker username already exists. Duplication is error.");
				
				String testUsername = ucc.setGoodUsername(neo.getUserName());
				if(testUsername==null) throw new RuntimeException("New worker. Username not good.");
				if(testUsername.trim().length()==0) throw new RuntimeException("New worker. Username not good.");
				...
				try {
					...
					throw new RuntimeException("Worker update username not successfull."); 
				}
			}
			...
			messageBean.setType(MessageType.SUCCESS);
			messageBean.setException(null);
			messageBean.setMessage("Измјена корисничког имена за запосленог "+userName+" је успјешна.");
		}catch(Exception ex) {
			MessageBean messageBean = DesignBeansGenerator.messageBean(session); 
			messageBean.setType(MessageType.ERROR);
			messageBean.setException(ex);
			messageBean.setMessage("Измјена корисничког имена за запосленог "+userName+" није успјешна.");
		}
	}
	
	public void updatePasswordWorker() {
		try {
			Worker w = current;
			if(w==null) throw new RuntimeException("Worker not found.");
			if(AppLogicCenter.appLogicCenter.getEngine().getDataSource().get(w.getUserName())==null)
			...
			UserCredentialsController ucc = new UserCredentialsController();
			Worker neo = getWorker();  
			if(neo==null) throw new RuntimeException("Worker not found");
			...
			if(testPassword==null) throw new RuntimeException("Worker. Password not good.");
			if(testPassword.trim().length()==0) throw new RuntimeException("Worker. Password not good.");
			
			try {
				if(!AppLogicCenter.appLogicCenter.getController().updatePassword(w.getUserName(), w.getPassword(), neo.getPassword()))
					throw new RuntimeException();
			}catch(Exception ex) {
				throw new RuntimeException("Worker current password not correct."); 
			}
			...
			messageBean.setType(MessageType.SUCCESS);
			messageBean.setException(null);
			messageBean.setMessage("Измјена лозинке за запосленог "+userName+" је успјешна.");
		}catch(Exception ex) {
			MessageBean messageBean = DesignBeansGenerator.messageBean(session); 
			messageBean.setType(MessageType.ERROR);
			messageBean.setException(ex);
			messageBean.setMessage("Измјена лозинке за запосленог "+userName+" није успјешна.");
		}
	}
	...
	public void updatePassword() {
		try {
			...
			try {
				...
				throw new RuntimeException("Worker update password not successfull."); 
			}
